  ...
  while (NRemain > 0) {
    ...
    if (!Embd.empty()) {
      ...
      if (static_cast<uint64_t>(Embd.size()) > MaxTokensListSize) {
        if (GraphRef.EnableLog) {
          spdlog::info("[WASI-NN] GGML backend: the prompt is too long. Your "
                       "input has {} tokens. Please reduce it to {} tokens."sv,
      ...
      if (NPast + static_cast<uint64_t>(Embd.size()) > NCtx) {
        if (GraphRef.EnableLog) {
          spdlog::info(
              "[WASI-NN] GGML backend: the context if full ({} / {} tokens). "
              "Please increase your ctx-size."sv,
      ...
      for (int I = 0; I < static_cast<int>(Embd.size());
        ...
        if (Status == 1) {
          spdlog::error(
              "[WASI-NN] GGML backend: failed to llama_decode: try reducing "
              "the size of the batch or increasing the size of context"sv);
          return ErrNo::RuntimeError;
        }
        if (Status < 0) {
          spdlog::error(
              "[WASI-NN] GGML backend: failed to llama_decode: internal "
              "fatal error. Please open an issue on GitHub"sv);
          return ErrNo::RuntimeError;
        }
...
Expect<ErrNo> computeSingle(WasiNNEnvironment &Env,
  ...
  if (static_cast<uint64_t>(CxtRef.LlamaInputs.size()) > MaxTokensListSize) {
    if (GraphRef.EnableLog) {
      spdlog::info(
          "[WASI-NN] GGML backend: the prompt is too long. Your input has {} "
          "tokens. Please reduce it to {} tokens."sv,
          CxtRef.LlamaInputs.size(), MaxTokensListSize);
    }
    return ErrNo::PromptTooLong;
  }
